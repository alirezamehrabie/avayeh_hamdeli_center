            ...
            @if($dashboardMode)
                <div>
                    <button type="button" @click="openMenu = openMenu === 'services' ? '' : 'services'"
                            class="flex items-center justify-between w-full px-4 py-2.5 rounded-lg hover:bg-indigo-800 {{ $isActive(['define-services', 'service-list', 'service-delivery']) ? 'bg-indigo-700' : '' }}">
                        <div class="flex items-center">
                            <svg class="w-5 h-5 ml-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        @if($dashboardMode)
            <div>
                <button type="button" @click="openMenu = openMenu === 'system-settings' ? '' : 'system-settings'"
                        class="flex items-center justify-between w-full px-4 py-2.5 rounded-lg hover:bg-indigo-800 {{ $isActive(['system-settings-user-management', 'system-settings-user-account']) ? 'bg-indigo-700' : '' }}">
                    <div class="flex items-center">
                        <svg class="w-5 h-5 ml-3" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                             stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">

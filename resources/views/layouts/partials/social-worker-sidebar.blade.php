...
>
    ...
    <nav class="flex-1 space-y-2">
        <a href="{{ route('social-worker.dashboard') }}" x-data="{ nav: false }" @click="nav = true" @pageshow.window="if ($event.persisted) nav = false" class="flex items-center rounded-lg px-4 py-2.5 transition-all duration-200 {{ $isServiceDeliveryActive ? 'bg-indigo-700 text-white shadow-sm' : 'text-indigo-100 hover:bg-indigo-800 hover:text-white' }}">
            ...
            <span :class="nav && 'opacity-60'">تحویل خدمت</span>
            <x-sidebar.loading-dots x-show="nav" x-cloak class="mr-auto text-indigo-200" />
        </a>

        <a href="{{ route('social-worker.attendance') }}" x-data="{ nav: false }" @click="nav = true" @pageshow.window="if ($event.persisted) nav = false" class="flex items-center rounded-lg px-4 py-2.5 transition-all duration-200 {{ $isAttendanceActive ? 'bg-indigo-700 text-white shadow-sm' : 'text-indigo-100 hover:bg-indigo-800 hover:text-white' }}">
            <svg class="ml-3 h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <span :class="nav && 'opacity-60'">حضور و غیاب</span>
            <x-sidebar.loading-dots x-show="nav" x-cloak class="mr-auto text-indigo-200" />
        </a>

        <a href="{{ route('social-worker.delivery-history') }}" x-data="{ nav: false }" @click="nav = true" @pageshow.window="if ($event.persisted) nav = false" class="flex items-center rounded-lg px-4 py-2.5 transition-all duration-200 {{ $isDeliveryHistoryActive ? 'bg-indigo-700 text-white shadow-sm' : 'text-indigo-100 hover:bg-indigo-800 hover:text-white' }}">
            <svg class="ml-3 h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <span :class="nav && 'opacity-60'">تاریخچه تحویل</span>
            <x-sidebar.loading-dots x-show="nav" x-cloak class="mr-auto text-indigo-200" />
        </a>

        <a href="{{ route('social-worker.user-account') }}" x-data="{ nav: false }" @click="nav = true" @pageshow.window="if ($event.persisted) nav = false" class="flex items-center rounded-lg px-4 py-2.5 transition-all duration-200 {{ $isUserAccountActive ? 'bg-indigo-700 text-white shadow-sm' : 'text-indigo-100 hover:bg-indigo-800 hover:text-white' }}">
            <svg class="ml-3 h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">

    ...
    <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6 xl:p-7">
        <div class="mb-6 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            ...
            <div class="min-w-0 flex-1">
                <h1 class="text-xl font-bold text-slate-800 sm:text-2xl xl:text-[1.7rem]">گزارش‌گیری پیشرفته سرپرستان</h1>
        ...
        <div class="mb-4 rounded-xl border border-slate-200 bg-slate-50 p-4">
            ...
            <div class="flex flex-col gap-2 lg:flex-row lg:items-center">
                <input type="text" wire:model.live.debounce.300ms="globalSearch" class="w-full min-w-0 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm focus:border-indigo-300 focus:ring focus:ring-indigo-100 lg:flex-1" placeholder="نام، کد ملی، موبایل یا کد خانوار...">
            ...
            <div class="flex flex-col gap-3 lg:flex-row lg:flex-wrap lg:items-center lg:justify-between">
                <div class="min-w-0">
                    ...
                    <p class="mt-1 text-sm font-bold text-slate-800">
                        ...
                        <span class="rounded-md bg-white px-2 py-0.5 text-sky-700">{{ number_format($this->guardians->total()) }}</span>
                    </p>
                </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    ...
                    @forelse($this->guardians as $guardian)
                        <tr wire:key="advanced-guardian-row-{{ $guardian->id }}" wire:click="showGuardianInfo({{ $guardian->id }})" class="cursor-pointer transition hover:bg-slate-50">
                            @foreach($visibleColumns as $columnKey)
                                <td class="whitespace-nowrap px-4 py-3 text-xs text-slate-700">
                                    @switch($columnKey)
                                        ...
                                        @case('responsible_social_worker')
                                            {{ $guardian->socialWorker?->full_name ?? '-' }}
                                            @break
                                        @case('guardian_birth_date')
                                            {{ $guardian->guardian_formatted_birth_date ?? '-' }}
                                            @break
                                        @case('guardian_age')
                                            {{ $guardian->guardian_age !== null ? $guardian->guardian_age . ' سال' : '-' }}
                                            @break
                                        @case('occupation')
                                            {{ $guardian->occupation?->name ?? '-' }}
                                            @break
                                        @case('job_type')
                                            {{ $guardian->jobType?->name ?? '-' }}
                                            @break
                                        @case('economic_decile')
                                            {{ $guardian->economic_decile ? 'دهک ' . $guardian->economic_decile : '-' }}
                                            @break
                                        @case('average_income')
                                            {{ $guardian->average_income ? number_format((int) $guardian->average_income) : '-' }}
                                            @break
                                        @case('beneficiaries_count')
                                            {{ number_format($guardian->people_count ?? 0) }}
                                            @break
                                        @case('insurance_status')
                                            {{ $guardian->insurance_status ? 'بله' : 'خیر' }}
                                            @break
                                        @case('has_vehicle')
                                            {{ $guardian->has_vehicle ? 'بله' : 'خیر' }}
                                            @break
                                        @case('district')
                                            {{ $guardian->residence?->district?->name ?? '-' }}
                                            @break
                                        @case('housing_status')
                                            {{ $guardian->residence?->residenceStatus?->name ?? '-' }}
                                            @break
                                        @case('created_at')
                                            {{ optional($guardian->created_at)->format('Y/m/d') }}
                                            @break
                                        @default
                                            {{ data_get($guardian, $columnKey) ?? '-' }}
                                    @endswitch
                                </td>
                            @endforeach
                        </tr>
        ...
        @include('livewire.guardians.partials.guardian-details-modal')
    </div>
</div>

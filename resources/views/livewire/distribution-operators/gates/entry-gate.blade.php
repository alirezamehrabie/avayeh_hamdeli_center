...
<div class="space-y-6" dir="rtl">
    <div class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        @if($selectedService)
            ...
            <div class="sticky top-0 z-20 border-b border-emerald-100 bg-emerald-50/95 px-5 py-3 backdrop-blur supports-[backdrop-filter]:bg-emerald-50/80">
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div class="flex min-w-0 items-center gap-3">
                        <span class="inline-flex shrink-0 items-center gap-1.5 rounded-full bg-emerald-600 px-3 py-1 text-[11px] font-black text-white">
                            <span class="h-1.5 w-1.5 animate-pulse rounded-full bg-emerald-200"></span>
                            گیت ورود فعال
                        </span>
                        <div class="min-w-0">
                    ...
                    <button
                        ...
                        class="inline-flex w-fit shrink-0 items-center gap-1.5 rounded-full border border-emerald-200 bg-white px-3 py-1.5 text-xs font-bold text-emerald-700 transition hover:bg-emerald-100"
                    >
        ...
        @else
            ...
            <div
                ...
                class="p-4 sm:p-5"
            >
                {{-- Top row: Identity + Scanner side by side on desktop --}}
                <div class="grid gap-5 lg:grid-cols-2 mb-6">
                    ...
                    {{-- Column 2: Scanner camera + controls --}}
                    <div class="space-y-3">
                        {{-- Camera scanner --}}
                        <div class="relative h-[clamp(200px,30svh,320px)] overflow-hidden rounded-2xl border border-slate-200 bg-slate-950 landscape:max-h-[55svh]">
                            <div
                                wire:ignore
                                x-ref="scanner"
                            ...
                            >
                                ...
                                <p class="text-[11px] font-semibold text-slate-300">در صورت نبود دوربین، از «جستجوی دستی» سمت چپ استفاده کنید</p>
                            </div>
                        </div>
                ...
                {{-- Delivery method: who physically receives the items (recorded per subject per service) --}}
                <div class="mb-6 rounded-3xl border p-4 shadow-sm transition-colors lg:p-5 {{ $isProxyDelivery ? 'border-amber-300 bg-amber-50/50' : 'border-slate-200 bg-white' }}">
                    <div class="flex flex-wrap items-center justify-between gap-2">
                        ...
                        @if($isProxyDelivery && $this->proxyRecipientLabel !== '')
                            ...
                            تحویل به: {{ $this->proxyRecipientLabel }}
                            </span>
                        @endif
                    </div>

                    @if(! $lastScanResult)
                        <p class="mt-3 rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-4 py-3 text-center text-xs font-semibold text-slate-500">
                    ...
                    @else
                        <label class="mt-3 block cursor-pointer">
                            ...
                            <div class="flex items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-3 transition peer-checked:border-amber-400 peer-checked:bg-amber-50 peer-checked:ring-4 peer-checked:ring-amber-100 peer-focus-visible:ring-4 peer-focus-visible:ring-amber-200">
                ...
                <div class="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm lg:p-5">
                    ...
                    {{-- Desktop header --}}
                    <div class="mb-4 flex items-center justify-between max-lg:hidden">
                    ...
                    @else
                        ...
                        {{-- Confirm button --}}
                        <div class="sticky bottom-0 mt-4 -mx-1 bg-gradient-to-t from-white via-white to-transparent px-1 pb-1 pt-3">
                            <button
                                type="button"
                ...
                {{-- Mobile-only button to reopen categories --}}
                @if($lastScanResult)
                    <button
                        type="button"
                        @click="categoriesSheetOpen = true"
                        class="mt-4 flex w-full items-center justify-between gap-2 rounded-2xl border border-indigo-200 bg-indigo-50 px-4 py-3 text-sm font-bold text-indigo-700 transition hover:bg-indigo-100 lg:hidden"
                    >
                        <span class="flex items-center gap-2">

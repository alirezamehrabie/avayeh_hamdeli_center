        ...
        <div class="divide-y divide-slate-100">
            @forelse($this->incompletePeople as $person)
                @php
                    ...
                    $nextStep = $this->nextStepFor($person);
                    $reasonsBySection = $reasons->groupBy('section');
                    $reasonSeverityWeights = [
                        'critical' => 4,
                        'high' => 3,
                        'medium' => 2,
                        'low' => 1,
                    ];
                    $sectionSummaries = $reasonsBySection
                        ->map(function ($sectionReasons, $sectionLabel) use ($fieldPathLabels, $reasonSeverityWeights) {
                            $orderedReasons = $sectionReasons
                                ->sort(function ($left, $right) use ($reasonSeverityWeights) {
                                    $leftWeight = $reasonSeverityWeights[$left['severity']] ?? 0;
                                    $rightWeight = $reasonSeverityWeights[$right['severity']] ?? 0;
                            ...
                            return [
                                'label' => $sectionLabel,
                                'count' => $orderedReasons->count(),
                                'visible_items' => $orderedReasons
                                    ->take(3)
                                    ->map(fn ($reason) => $fieldPathLabels[$reason['field']] ?? $reason['label'])
                                    ->values(),
                                'hidden_count' => max(0, $orderedReasons->count() - 3),
                            ];
                        })
                        ->sortByDesc('count')
                        ->values();
                ...
                <article class="px-5 py-4">
                    <div class="flex flex-col gap-4">
                        ...
                        <div class="rounded-2xl border border-indigo-200 bg-indigo-50/80 px-4 py-3">
                            <div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                                <div class="min-w-0">
                                    <p class="text-xs font-semibold text-indigo-700">اقدام بعدی پیشنهادی</p>
                                    <p class="mt-1 text-sm font-bold text-slate-900">{{ $nextStep['headline'] }}</p>
                                    <p class="mt-1 text-xs leading-6 text-slate-600">{{ $nextStep['supporting_text'] }}</p>
                                </div>
                                ...
                                <div class="flex flex-wrap gap-2 lg:justify-end">
                                    @can('people-edit')
                                        <button
                                            ...
                                            class="inline-flex items-center justify-center rounded-lg border border-indigo-200 bg-white px-3 py-2 text-xs font-semibold text-indigo-700 transition hover:bg-indigo-100 focus:outline-none focus:ring-4 focus:ring-indigo-100"
                                        >
                                            {{ $nextStep['primary_action_label'] }}
                                        </button>
                                    @endcan
                        ...
                        <div>
                            <div class="mb-3 flex flex-wrap items-center justify-between gap-2">
                                <p class="text-xs font-semibold text-slate-500">فهرست فیلدهای ناقص</p>
                                <p class="text-[11px] text-slate-400">در هر بخش، مهم‌ترین موارد اول نمایش داده شده‌اند.</p>
                            </div>

                            <div class="rounded-2xl border border-slate-200 bg-slate-50/70 p-3">
                                <div class="grid grid-cols-1 gap-3 xl:grid-cols-2">
                                    @foreach($sectionSummaries as $sectionSummary)
                                        <section class="rounded-2xl border border-slate-200 bg-white px-3 py-3">
                                            <div class="flex items-center justify-between gap-3">
                                                <h4 class="text-xs font-bold text-slate-800">{{ $sectionSummary['label'] }}</h4>
                                                <span class="rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-semibold text-slate-600">
                                                    {{ number_format($sectionSummary['count']) }} نقص
                                                </span>
                                            </div>

                                            <div class="mt-3 flex flex-wrap gap-2">
                                                @foreach($sectionSummary['visible_items'] as $visibleItem)
                                                    <span class="rounded-full border border-slate-200 bg-slate-50 px-2.5 py-1 text-[11px] font-medium text-slate-700">
                                                        {{ $visibleItem }}
                                                    </span>
                                                @endforeach

                                                @if($sectionSummary['hidden_count'] > 0)
                                                    <span class="rounded-full border border-dashed border-slate-300 bg-white px-2.5 py-1 text-[11px] font-medium text-slate-500">
                                                        +{{ number_format($sectionSummary['hidden_count']) }} مورد دیگر
                                                    </span>
                                                @endif
                                            </div>
                                        </section>

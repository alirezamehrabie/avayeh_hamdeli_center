    ...
    <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6 xl:p-7">
        <div class="mb-6 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            ...
            <div class="min-w-0 flex-1">
                <h1 class="text-xl font-bold text-slate-800 sm:text-2xl xl:text-[1.7rem]">گزارش‌گیری پیشرفته مددکاران</h1>
                <p class="mt-1 text-sm text-slate-500">تحلیل عملکرد، حجم کاری و خدمات مددکاران با فیلترهای پویا و خروجی اکسل</p>
            </div>
        ...
        {{-- خلاصهٔ آماری لحظه‌ای روی کل نتایج فیلترشده --}}
        ...
        <div class="mb-5 grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-6">
            <div class="rounded-xl border border-indigo-100 bg-indigo-50/60 p-3">
                <p class="text-[11px] font-semibold text-indigo-700">مددکاران</p>
                <p class="mt-1 text-lg font-extrabold text-slate-800">{{ number_format($summary['workers']) }}</p>
            </div>
            <div class="rounded-xl border border-amber-100 bg-amber-50/60 p-3">
                <p class="text-[11px] font-semibold text-amber-700">خانوارهای تحت پوشش</p>
                <p class="mt-1 text-lg font-extrabold text-slate-800">{{ number_format($summary['households']) }}</p>
                <p class="mt-0.5 text-[10px] text-slate-500">میانگین {{ $summary['avg_households'] }} به ازای هر مددکار</p>
            </div>
            <div class="rounded-xl border border-cyan-100 bg-cyan-50/60 p-3">
                <p class="text-[11px] font-semibold text-cyan-700">مددجویان تحت پوشش</p>
                <p class="mt-1 text-lg font-extrabold text-slate-800">{{ number_format($summary['beneficiaries']) }}</p>
                <p class="mt-0.5 text-[10px] text-slate-500">میانگین {{ $summary['avg_beneficiaries'] }} به ازای هر مددکار</p>
            </div>
            <div class="rounded-xl border border-violet-100 bg-violet-50/60 p-3">
                <p class="text-[11px] font-semibold text-violet-700">خدمات تخصیص‌یافته</p>
                <p class="mt-1 text-lg font-extrabold text-slate-800">{{ number_format($summary['services']) }}</p>
            </div>
            <div class="rounded-xl border border-emerald-100 bg-emerald-50/60 p-3">
                <p class="text-[11px] font-semibold text-emerald-700">تحویل‌های انجام‌شده</p>
                <p class="mt-1 text-lg font-extrabold text-slate-800">{{ number_format($summary['deliveries']) }}</p>
            </div>
            <div class="rounded-xl border border-rose-100 bg-rose-50/60 p-3">
                <p class="text-[11px] font-semibold text-rose-700">ارزش تحویل‌ها (ریال)</p>
                <p class="mt-1 text-base font-extrabold text-slate-800">{{ number_format($summary['delivered_value']) }}</p>
            </div>
        </div>
        ...
        <div class="mb-4 rounded-xl border border-slate-200 bg-slate-50 p-4">
            ...
            <div class="flex flex-col gap-2 lg:flex-row lg:items-center">
                <input type="text" wire:model.live.debounce.300ms="globalSearch" class="w-full min-w-0 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm focus:border-indigo-300 focus:ring focus:ring-indigo-100 lg:flex-1" placeholder="نام، کد ملی، موبایل یا کد مددکار...">
        ...
        {{-- فیلترهای پیشرفته به‌صورت تاشو برای تمیزتر شدن رابط کاربری --}}
        <div x-data="{ advancedOpen: @js(count($filters) > 0) }" class="mb-4 rounded-xl border border-slate-200">
            <button type="button" @click="advancedOpen = !advancedOpen" class="flex w-full items-center justify-between gap-2 px-4 py-3 text-right">
                <span class="text-sm font-bold text-slate-700">فیلترهای پیشرفته</span>
                <span class="flex items-center gap-2">
                    @if(count($filters) > 0)
                        <span class="rounded-full bg-indigo-50 px-2 py-0.5 text-[11px] font-semibold text-indigo-700">{{ count($filters) }} فیلتر فعال</span>
                    @endif
                    <svg class="h-4 w-4 text-slate-500 transition-transform" :class="advancedOpen ? 'rotate-180' : ''" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M5.23 7.21a.75.75 0 011.06.02L10 11.17l3.71-3.94a.75.75 0 111.08 1.04l-4.25 4.5a.75.75 0 01-1.08 0l-4.25-4.5a.75.75 0 01.02-1.06z" clip-rule="evenodd"/></svg>
                </span>
            </button>

            <div x-show="advancedOpen" x-collapse class="border-t border-slate-200 p-4">
                            ...
                            @if(($filter['type'] ?? null) === 'date')
                                <div class="space-y-2">
                                    ...
                                    @if(($filter['mode'] ?? 'exact') === 'exact')
                                        <input type="text" wire:model.live="filters.{{ $index }}.exact" class="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs" placeholder="مثال: 1360/07/15">
                                    @endif

                                    @if(($filter['mode'] ?? null) === 'range')
                                        <div class="grid gap-2 md:grid-cols-2">
                                            <input type="text" wire:model.live="filters.{{ $index }}.from" class="rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs" placeholder="از تاریخ (1350/01/01)">
                                            <input type="text" wire:model.live="filters.{{ $index }}.to" class="rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs" placeholder="تا تاریخ (1370/12/29)">
                                        </div>
                                    @endif
                                    ...
                                    @if(($filter['mode'] ?? null) === 'year')
                                        <input type="number" wire:model.live="filters.{{ $index }}.year" class="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs" placeholder="سال">
                                    @endif
                                </div>
                            @endif
                ...
                <div class="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1.25fr)_minmax(0,1fr)]">
            ...
            <div class="flex flex-col gap-3 lg:flex-row lg:flex-wrap lg:items-center lg:justify-between">
                <div class="min-w-0">
                    ...
                    <p class="mt-1 text-sm font-bold text-slate-800">
                        تعداد مددکاران یافت‌شده:
                        <span class="rounded-md bg-white px-2 py-0.5 text-sky-700">{{ number_format($this->workers->total()) }}</span>
                    </p>
                </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    ...
                    @forelse($this->workers as $worker)
                        <tr wire:key="advanced-worker-row-{{ $worker->id }}" wire:click="showWorkerInfo({{ $worker->id }})" class="cursor-pointer transition hover:bg-slate-50">
                            @foreach($visibleColumns as $columnKey)
                                <td class="whitespace-nowrap px-4 py-3 text-xs text-slate-700">
                                    @switch($columnKey)
                                        ...
                                        @case('occupation')
                                            ...
                                            @break
                                        @case('academic_level')
                                            {{ $worker->academicLevel?->title ?? '-' }}
                                            @break
                                        @case('birth_date')
                                            {{ $worker->formatted_birth_date ?? '-' }}
                                            @break
                                        @case('age')
                                            {{ $worker->age !== null ? $worker->age . ' سال' : '-' }}
                                            @break
                                        @case('start_date')
                                            {{ $worker->formatted_start_date ?? '-' }}
                                            @break
                                        @case('households_count')
                                            {{ number_format($worker->guardians_count ?? 0) }}
                                            @break
                                        @case('beneficiaries_count')
                                            {{ number_format($worker->people_count ?? 0) }}
                                            @break
                                        @case('services_count')
                                            {{ number_format($worker->services_count ?? 0) }}
                                            @break
                                        @case('deliveries_count')
                                            {{ number_format($worker->service_deliveries_count ?? 0) }}
                                            @break
                                        @case('delivered_value')
                                            {{ number_format((int) ($worker->service_deliveries_sum_delivered_total_value ?? 0)) }}
                                            @break
                                        @case('is_active')
                                            @if($worker->is_active)
                                                <span class="rounded-full bg-emerald-50 px-2 py-0.5 text-[11px] font-semibold text-emerald-700">فعال</span>
                                            @else
                                                <span class="rounded-full bg-rose-50 px-2 py-0.5 text-[11px] font-semibold text-rose-700">غیرفعال</span>
                                            @endif
                                            @break
                                        @case('created_at')
                                            {{ optional($worker->created_at)->format('Y/m/d') }}
                                            @break
                                        @default
                                            {{ data_get($worker, $columnKey) ?? '-' }}
                                    @endswitch
                                </td>
                            @endforeach
                        </tr>
